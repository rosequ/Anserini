...
if __name__ == "__main__":
  ...
  parser.add_argument('--create-qrel', help='create qrel file', action="store_true")

  args = parser.parse_args()

  if (args.model == "sm" and not args.w2v_cache and not args.qa_model_file):
    print("Pass the word embeddings cache fiela and the model file")
    parser.print_help()
    exit()
  ...
  if args.eval == 'pattern' and args.create_qrel:
    ...
    for qid, question in zip(answers.keys(), questions):
      ...
      if args.eval == 'jaccard':
        ...
        for key, value in scored_candidates.items():
          ...
          # check if the answer already exists in TrecQA test set
